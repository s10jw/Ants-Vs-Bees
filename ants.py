                ...
                if place.bees:
                    if place.is_hive:
                        ...
                        if self.lower_bound <= index <= self.upper_bound:
                            ...
                            index += 1
                            return helper(place.entrance, index)
                else:
                    index += 1
                    return helper(place.entrance, index)
            except:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        if self.ant_contained is None and isinstance(other, Ant) and other.is_container is False:
            return True
        else:
            return False
...
class QueenAnt(ScubaThrower):
    ...
    def construct(cls, gamestate):
        ...
        """
        if gamestate.queen_posted is False:
            gamestate.queen_posted = True
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        def bee_action(gamestate):
            """Throw a leaf at the nearest Bee in range."""
            # check if entry can be returned to normal
            if target.health > 0 and target.turns >= 5:
                target.action = types.MethodType(Bee.action, target)
            ...
            target.turns += 1
        if target is not None:
            setattr(target, 'turns', 0)
            target.action = lambda *args, **kwargs: bee_action(*args, **kwargs)
